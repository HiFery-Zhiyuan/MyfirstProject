# -*- coding: utf-8 -*-
"""
Created on Fri Mar   10 13:47 2023
...
def case14_CE():
    ...
    df = pd.read_excel('14casecarbon_emission.xlsx')
...
def matrix_PN(PB, PG):
    ...
    if np.any(temp[0] == 0):
        ...
        # print('#---------------------------------#')
        # print('存在注入功率为零的节点。节点编号：', position[0][0], sep='\n')
        # print('节点功率注入设置 0.001MW ')
        # print('#---------------------------------#')
        temp[0][position[0][0]]=0.001
...
def create_table_3(branch_flows, PB_, RPB_, RB_):
    ...
    df2 = pd.DataFrame(table3_2, columns=['有功潮流PB', '碳流密度RPB', '碳流率RB'])

    # double check the power flow values of the branches, 是否为零
    zero_list = df2[df2['有功潮流PB'] < 0.001].index.to_list()
    if zero_list:
        for i in range(len(zero_list)):
            df2.loc[zero_list[i], '碳流密度RPB'] = 0

        table3 = pd.concat([branch_orders, df2], axis=1, ignore_index=False)
    else:
        table3 = pd.concat([branch_orders, df2], axis=1, ignore_index=False)
    return table3
